...
1. shell 中可以操作的指令或可执行文件。
2. 系统核心可使用的函数与工具等。
3. 一些常用的函数 (function) 与库 (library) ，大部分为 C的库 (libc)。
...
tar命令是一个古老的命令，其最初目的是用于将文件备份到磁带上，这也是命令名称的来源，tar即“tape archive”的缩写。
...
* `-exec command {} \;`：将查到的文件执行command操作，`{}` 和` \;`之间有空格。用`{} `代表查找到的文件，用`\;`表示结束。
...
1. 根用户 (root)：具有系统全部权限的用户。UID=0
2. 普通用户：其使系统的权限受到一定制。UID>=100（或者1000）
...
这种支持也称为“globbing”（因为它最初被实现为一个名为 /etc/glob 的程序），让你能够使用通配符模式指定多个文件。
...
本质上来说，管道就是一种特殊的重定向，也就是对一个命令的输出进行管道连接（即重定向），用作下一个命令的输入。
...
1. 一般模式（NORMAL）：会在屏幕的底部显示文件名或者是空白。在打开vim 时默认处于一般模式。
2. 输入模式：底部会显示` –INSERT–`。
3. 可视模式：底部显示的是`–VISUAL–`。
...
* `J`：删除当前行的换行符。
* `u`：撤销最后一次编辑。
* `Ctrl+r`：如果你撤销过 头了，用来取消撤销过多带的后果。
* `ZZ`也即`Shift+z+z `：退出 vim ，如果文件作过改 ，如果文件作过改 ，保存文件并退出 。如果文件没有作过改动，就直接 退出 vim 。
